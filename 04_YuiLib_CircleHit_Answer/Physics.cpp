...
void Physics::Update()
{
	...
	for (auto item : collidables)
	{
		...
		DebugDraw::DrawCircle(nextPos, item->radius, 0xff00ff);
				...
				if (objA != objB)
				{
					...
					if (atobLength < objA->radius + objB->radius)
					{
						...
						if(player && enemy)
						{
							...
							float awayDist = player->radius + enemy->radius + 0.0001f;
							//VECTOR oldEnemyPos = enemy->nextPos;
							...
							DebugDraw::DrawLine(oldEnemyPos, enemy->rigidbody.GetPos());
...
#if _DEBUG
		// 補正後の位置をデバッグ表示
		DebugDraw::DrawLine(item->rigidbody.GetPos(), item->nextPos, 0xffffff);
		DebugDraw::DrawCircle(item->nextPos, item->radius, 0xffff00);
#endif
