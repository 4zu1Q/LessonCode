...
void Physics::Update()
{
	...
	for (auto& item : collidables)
	{
		...
		else if (item->colliderData->GetKind() == ColliderData::Kind::Line2D)
		{
			ColliderDataLine2D* lineData;
			lineData = static_cast<ColliderDataLine2D*>(item->colliderData);
...
bool Physics::IsCollide(const Collidable* objA, const Collidable* objB) const
{
	...
	// お互いの距離が、それぞれの半径を足したものより小さければ当たる
	if (objA->colliderData->GetKind() == ColliderData::Kind::Circle2D)
	{
		if (objB->colliderData->GetKind() == ColliderData::Kind::Circle2D)
		{
	...
	else if (objA->colliderData->GetKind() == ColliderData::Kind::Line2D)
	{
		if (objB->colliderData->GetKind() == ColliderData::Kind::Circle2D)
		{
			auto objAColliderData = static_cast<ColliderDataLine2D*>(objA->colliderData);
			auto objBColliderData = static_cast<ColliderDataCircle2D*>(objB->colliderData);

			float i = Segment_Point_MinLength(objAColliderData->startPoint, objAColliderData->endPoint, objB->rigidbody.GetPos());

			return ( i < objBColliderData->radius);
		}
...
{
	// お互いの距離が、それぞれの半径を足したものより小さければ当たる
	if (primary->colliderData->GetKind() == ColliderData::Kind::Circle2D)
	{
		if (secondary->colliderData->GetKind() == ColliderData::Kind::Circle2D)
		{
...
void Physics::FixPosition()
{
	for (auto& item : collidables)
	{
		...
		{
			auto itemLineData = static_cast<ColliderDataLine2D*>(item->colliderData);
		}
#endif
